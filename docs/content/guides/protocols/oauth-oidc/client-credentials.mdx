...
| `aud` claim | The supplied resource server interface identifier, or the identifier of the configured default resource server interface for a permission-bearing request without `resource`. A scopeless request without `resource` uses the application's `token.accessToken.defaultAudience`, falling back to `client_id` |
...
:::note
When requesting permission scopes, the client must include `resource` unless a default resource server interface is configured globally. A scopeless request does not require either setting and uses the application's `token.accessToken.defaultAudience` (falling back to `client_id`) as its audience.
:::
