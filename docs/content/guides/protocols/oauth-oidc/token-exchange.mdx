...
| `resource` parameter | RFC 8707 resource indicator. The issued access token is bound to the resource server interface with this identifier. Permission-bearing requests use the configured default resource server interface when it is omitted. OIDC-only or scopeless requests use the application's `token.accessToken.defaultAudience`, falling back to `client_id` |
