...
| Resource binding (RFC 8707) | An optional `resource` binds the request to one resource server interface at initiation. When a permission scope is requested without `resource`, the configured default resource server interface applies; with no default, the request is rejected with `invalid_target`. The access token's `aud` is the bound interface identifier, and its permissions are limited to the owning resource server. OIDC-only requests without `resource` use the application's `token.accessToken.defaultAudience`, falling back to `client_id`. A `resource` supplied when polling must match the binding |
