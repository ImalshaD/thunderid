...
**Resource Indicators for OAuth 2.0** ([RFC 8707](https://datatracker.ietf.org/doc/html/rfc8707)) lets a client tell <ProductName /> *which* resource server interface the access token is for. The client passes a single `resource` parameter. <ProductName /> matches it to a registered interface identifier, narrows the granted permission scopes to those owned by the interface's resource server, and writes the interface identifier into the token's `aud` claim. When permission scopes are requested without `resource`, <ProductName /> uses the identifier of the configured default resource server interface. If neither target is available, token issuance fails with `invalid_target`. OIDC-only and scopeless requests without `resource` are not bound to a resource server, so their audience is the application's configured default audience (`token.accessToken.defaultAudience`), or the `client_id` when it is unset.
...
For permission-bearing authorization requests, resource binding is resolved per request and is honored across single sign-on: when a request is satisfied by an existing SSO session, its permission scopes are evaluated against that request's own `resource` (or the configured default resource server interface), so a session established for one resource server never leaks permission scopes to a different one on SSO reuse. OIDC-only and scopeless requests without `resource` remain unbound to a resource server and use the `client_id` as the audience.
...
The issued access token carries the resource server interface identifier as its `aud`:
...
| Resolution | The URI is matched to a registered resource server interface identifier |
| Unknown identifier | Request rejected with `invalid_target` |
| No `resource` | Permission-bearing requests use the identifier of the configured default resource server interface and return `invalid_target` if it is unset. OIDC-only and scopeless requests use the application's `token.accessToken.defaultAudience`, or `client_id` when it is unset |
...
| No `resource`, permission scopes requested | The identifier of the configured default resource server interface. If no default is configured, the request is rejected with `invalid_target`. |
