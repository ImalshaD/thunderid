...
Access tokens with permission scopes are bound to one resource server through a selected interface. Send `resource=https://api.example.com/...` on the authorization or token request when the client knows the target interface, or configure a default resource server interface for permission-bearing requests that omit `resource`. OIDC-only or scopeless requests without `resource` use the application's `token.accessToken.defaultAudience`, falling back to `client_id`.
