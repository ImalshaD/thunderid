...
| Tokens rejected with "audience mismatch" | The authorization server is not receiving a `resource` parameter and falls back to a default resource server interface whose identifier does not match <ProductName />'s expected audience | Configure the client to send `resource=<server-url>` in the authorization request, or configure the resource server interface with the expected identifier as the default. |
