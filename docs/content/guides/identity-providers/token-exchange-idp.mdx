...
The issued token's `sub` claim contains the external user's subject identifier from the original token. For permission-bearing requests, its `aud` claim is the explicit resource server interface identifier or the identifier of the configured default resource server interface. An OIDC-only or scopeless request without `resource` uses the application's default audience (`token.accessToken.defaultAudience`), or the `client_id` when it is unset.
