...
| `aud` | The resolved resource server interface identifier, or the default audience (falling back to Client ID) for a scopeless request without `resource`. | The resolved resource server interface identifier, or the default audience (falling back to Client ID) when no permission scopes or `resource` are present. |
...
| `aud` | Matches the identifier of your resource server interface. |
