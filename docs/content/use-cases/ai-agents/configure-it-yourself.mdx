...
2. **Create the `wayfinder` resource server.**
   ...
   Configure the booking resource server interface as the default for Wayfinder web sign-in tokens. The web application requests `booking:*` permissions without a `resource`, so the default must be an interface of the resource server that defines those permissions, and its identifier becomes the token audience. The chat widget targets `http://localhost:8790/chat` explicitly.

   ```bash
   curl -kL -X PUT https://localhost:8090/server-config/defaultResourceServerInterface \
     -H 'Content-Type: application/json' \
     -H 'Authorization: Bearer <access-token>' \
     -d '{ "resourceServerInterfaceId": "<wayfinder-rs-interface-id>" }'
   ```
