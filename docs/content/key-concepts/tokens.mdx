...
| `aud` | The audience: one resource server interface identifier, or the application's default audience (`token.accessToken.defaultAudience`, falling back to `client_id`) for an unbound OIDC-only or scopeless token. |
...
- **With `resource` parameter:** The `aud` claim contains the identifier of the targeted resource server interface.
- **Without `resource`, with permission scopes:** <ProductName /> uses the identifier of the configured default resource server interface. If no default is configured, token issuance fails with `invalid_target`.
...
Resource servers that validate tokens should require `aud` to match the identifier of the interface through which they are accessed.
