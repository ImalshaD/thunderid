...
function deriveInitialValues(node: SelectedNode | null): {name: string; description: string} {
  if (!node) return {name: '', description: ''};
  if (node.type === 'server') {
    const rs = node.data;
    return {name: rs.name, description: rs.description ?? ''};
  }
...
function DetailForm({selectedNode, resourceServer, onRefresh}: DetailFormProps): JSX.Element {
      ...
      updateRs.mutate(
        {
          ...
          data: {name, description: description || null, ouId: resourceServer.ouId},
        },
        {
          onSuccess: () => {
  ...
  const resolveNodeTypeLabel = (): string => {
    ...
    if (selectedNode.type === 'resource') return t('resourceServers:detail.types.resource', 'Resource');
